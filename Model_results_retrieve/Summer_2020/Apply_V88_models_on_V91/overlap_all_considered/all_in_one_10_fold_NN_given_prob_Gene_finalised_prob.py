...
'''For SITE selection'''
if SITE_MUST:
	clean_must_sat_dir_part_save_parts_main='Fully_clean/SITE/'
else:
	clean_must_sat_dir_part_save_parts_main='Fully_clean/W_OUT_SITE/'
if SITE_MUST:
	clean_must_sat_dir_part_save_parts='Overlapped_allowed_all/SITE/'
else:
	clean_must_sat_dir_part_save_parts='Overlapped_allowed_all/W_OUT_SITE/'
...
'''
 this directory uses Fusion overlapped weighted primnary structures
 and Leavng ONGO and TSG overalpped PDBs
'''
...
main_dir =''.join([ "C:/Users/nishy/Documents/Projects_UL/Continues BIBM/a_Results_for_model_train/Tier_1/clean_NN_results/2020_k_fold_results/",clean_must_sat_dir_part_save_parts_main,threh_dir,amino_acid_dir])
...
nn_probability_overlapped_PDB_results=''.join([main_dir,'T_1_T_2/overlapped_PDB_results/',model_name])
...
#%%
os.chdir('/')
if not os.path.exists(nn_probability_overlapped_PDB_results):
   os.makedirs(nn_probability_overlapped_PDB_results)
os.chdir(nn_probability_overlapped_PDB_results)
print("Place the models' overlapped PDB results here")
...
T_1_T_2_obj = retrieve_comp_results_not_valid_10_fold_NN(model_name,nn_probability,nn_probability_left_pdb_part,nn_probability_overlapped_PDB_results,saving_dir_NN_retrieved)
...
def gene_seprated_results_finalise_overlapped_SITE_MUST_old_threh_T_1_T_2(name,T1_T2_test_prob_dic,saving_dir_part,SITE_MUST,num_amino_acids,Tier=1,straight_save=True):
    ...
    """
    if SITE_MUST:
        unigene_load_dir = ''.join(['C:/Users/nishy/Documents/Projects_UL/Continues BIBM/Uniprot_Mapping/Spring_2020/weights_assign_methods/Overlapped_allowed_all/T_',str(Tier),'/SITE/before_thresh/',str(num_amino_acids),'_aminoacid'])
        os.chdir('/')
...
#    """FOR clean new_ran_clean"""
    if Version_new:
        ONGO_test_gene_direct_prob, ONGO_test_fin_gene_prob, ONGO_test_Ensamble_gene_prob = gene_seprated_results_finalise_overlapped_SITE_MUST_old_threh_T_1_T_2("ONGO",T1_T2_test_prob_dic,saving_dir_primary_seq_part,SITE_MUST,num_amino_acids)
        TSG_test_gene_direct_prob, TSG_test_fin_gene_prob, TSG_test_Ensamble_gene_prob = gene_seprated_results_finalise_overlapped_SITE_MUST_old_threh_T_1_T_2("TSG",T1_T2_test_prob_dic,saving_dir_primary_seq_part,SITE_MUST,num_amino_acids)
        Fusion_test_gene_direct_prob, Fusion_test_fin_gene_prob, Fusion_test_Ensamble_gene_prob = gene_seprated_results_finalise_overlapped_SITE_MUST_old_threh_T_1_T_2("Fusion",T1_T2_test_prob_dic,saving_dir_primary_seq_part,SITE_MUST,num_amino_acids)
    else:
        print("Go through OLD")
        ONGO_test_gene_direct_prob, ONGO_test_fin_gene_prob, ONGO_test_Ensamble_gene_prob = gene_seprated_results_finalise_overlapped_SITE_MUST_old_threh_T_1_T_2("ONGO",saving_dir_NN_retrieved,saving_dir_primary_seq_part,threshold_dir_sel,SITE_MUST,num_amino_acids)
        TSG_test_gene_direct_prob, TSG_test_fin_gene_prob, TSG_test_Ensamble_gene_prob = gene_seprated_results_finalise_overlapped_SITE_MUST_old_threh_T_1_T_2("TSG",saving_dir_NN_retrieved,saving_dir_primary_seq_part,threshold_dir_sel,SITE_MUST,num_amino_acids)
        Fusion_test_gene_direct_prob, Fusion_test_fin_gene_prob, Fusion_test_Ensamble_gene_prob = gene_seprated_results_finalise_overlapped_SITE_MUST_old_threh_T_1_T_2("Fusion",saving_dir_NN_retrieved,saving_dir_primary_seq_part,threshold_dir_sel,SITE_MUST,num_amino_acids)
...
#        else:
        ...
        test_gene_direct_prob, test_fin_gene_prob, test_Ensamble_gene_prob = gene_seprated_results_finalise_overlapped_SITE_MUST_old_threh_T_1_T_2(name,T1_T2_test_prob_dic,saving_dir_primary_seq_part,SITE_MUST,num_amino_acids,Tier=Tier)
#%% 
'''
