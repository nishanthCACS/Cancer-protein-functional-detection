...
def fully_cleaned_count(main_directory):
    ...
    all_pdbs_train = ONGO_pdbs+TSG_pdbs+Fusion_pdbs
    all_pdbs_train = list(set(all_pdbs_train))
    all_pdbs_test=list(set(test_list_ids))
    
    all_pdbs = all_pdbs_train+all_pdbs_test
    ...
    fully_clean_count_dic['train_all']=len(list(set(all_pdbs_train)))
    fully_clean_count_dic['test_all']=len(list(set(all_pdbs_test)))
...
og_all_left_include=og_all+left_PDbs_OG
...
tsg_all_left_include=pdbs_tsg+left_PDbs_TSG

name = "Fusion"
left_PDbs_Fusion,not_there_fusion,train_unigene_details_fusion,train_pdb_ids_details_fusion,test_pdb_ids_details,test_unigene_details_fusion,test_pdb_ids_details_fusion = chk_train_PDBs_list(name,SITE)
train_pdb_ids_details_Fusion_all=list(set(sum(train_pdb_ids_details_fusion,[])))
test_pdb_ids_details_Fusion_all=list(set(sum(test_pdb_ids_details_fusion,[])))
fusion_all=train_pdb_ids_details_Fusion_all+test_pdb_ids_details_Fusion_all
fusion_all_left_include=fusion_all+left_PDbs_Fusion
...
train_all=list(set(train_all))
test_all=list(set(test_all))
print("Processed all Train: ",len(train_all))
print("Processed all Test: ",len(test_all))

count_overlap_train_test=0
for pdb in train_all:
    if pdb in test_all:
        count_overlap_train_test=count_overlap_train_test+1
print("Count overlapping Train and Test: ",count_overlap_train_test)
#%%
'''
...
#%%
'''
To count the overlapped PDBs whicha re left due to overlap
'''
def over_lap_count_not_there_genes(list_1,list_2,list_3):
    list_all=list_2+list_3
    overlap_cout=[]
    for i in range(0,len(list_1)):
        over_lap=0
        for pdb in list_1[i][1]:
            if pdb in list_all:
                over_lap=over_lap+1
        overlap_cout.append([list_1[i][0],over_lap])    
    return overlap_cout

over_count_OG=over_lap_count_not_there_genes(not_there,tsg_all_left_include,fusion_all_left_include)
over_count_TSG=over_lap_count_not_there_genes(not_there_tsg,og_all_left_include,fusion_all_left_include)
over_count_Fusion=over_lap_count_not_there_genes(not_there_fusion,tsg_all_left_include,og_all_left_include)

##%% 
'''
Findout the Fusion 50 overlap groups number
'''
#gene_group_overlap_details=[]
#for i in range(0,len(train_pdb_ids_details_fusion)):
#    count=0
#    for pdb in train_pdb_ids_details_fusion[i]:
#        if pdb in overlaped_all:
#            count=count+1
#    gene_group_overlap_details.append([len(train_pdb_ids_details_fusion[i]),count])
#    if count>0:
#        print(train_unigene_details_fusion[i])
#        print(" Actual: ",len(train_pdb_ids_details_fusion[i]))
#        print("Overlapped: ",count )
##gene_group_overlap_details=[]
##for i in range(0,len(test_pdb_ids_details_fusion)):
##    count=0
##    for pdb in train_pdb_ids_details_fusion[i]:
##        if pdb in overlaped_all:
##            count=count+1
##    gene_group_overlap_details.append([len(test_pdb_ids_details_fusion[i]),count])