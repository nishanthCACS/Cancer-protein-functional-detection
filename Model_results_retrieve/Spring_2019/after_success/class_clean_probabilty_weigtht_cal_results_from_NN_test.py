# -*- coding: utf-8 -*-
"""
Created on %(15-Nov-2018) 21.53Pm
...
class clean_probability_weight_cal_results_from_NN:
    """
    ...
    def __init__(self,working_dir_fraction, working_dir_NN_prob, unigene_test_dir, saving_dir_part,name,name_csv, minimum_prob_decide= 0.25):
        ...
        self.saving_dir =  "".join([saving_dir_part,'/',name])# to save the results
#        self.saving_dir =  "".join([saving_dir_part,'_',name])# to save the results
        self.unigene_test_dir = unigene_test_dir# to force the methods only choose the methods corresponding to testing UniGene_ids 
        self.minimum_prob_decide = minimum_prob_decide
        self.name = name
        self.saving_dir_part = saving_dir_part
    ...
    def load_method_results(self):  
        ...
        # to read the 
        os.chdir('/')
        os.chdir(self.unigene_test_dir) 
        if  self.name_csv[-5] =='l':
            print("choosing cleaned test unigene pikle of ",self.name)
            test_pickle = "".join([name,'_test_unoverlap_uni_gene.p'])
            test_gene_list = pickle.load(open(test_pickle,"rb"))
        else:
            test_pickle = "".join([name,'_test_uni_gene.p'])
#            test_pickle = "".join([name,'_train_unigene_details.p'])
            test_gene_list = pickle.load(open(test_pickle,"rb"))
        ...
        for n in weight_initialize_gene_from_PDB_seq_length:
            if n[len(name)+1:len(name)+7] == 'direct':
                ...
                if name == "ONGO":
                    ...
                    method_pikles_UniGene.append(n[len(name)+10:-2])   
    ...
    def calculate_prob(self):                
        ...
        for i in range(0,len(direct_pikles)):
            ...
            if  dir_1[1][0] not in PDB_id:
                print("")
                print(dir_1[1][0],"-----missing PDB_id for direct hit")
                print("")
    ...
    def documentation_prob(self):
        ...
        os.chdir('/')
        os.chdir(self.saving_dir_part)
        pickle.dump(gene_direct_prob, open( ''.join([self.name,"_T_1_train_gene_direct_prob.p"]), "wb" ) )  
        pickle.dump(fin_gene_prob, open( ''.join([self.name,"_T_1_train_fin_gene_prob.p"]), "wb" ) )  
        pickle.dump(Ensamble_gene_prob, open( ''.join([self.name,"_T_1_train_Ensamble_gene_prob.p"]), "wb" ) )  
#        pickle.dump(gene_direct_prob, open( ''.join([self.name,"_T_1_test_gene_direct_prob.p"]), "wb" ) )  
#        pickle.dump(fin_gene_prob, open( ''.join([self.name,"_T_1_test_fin_gene_prob.p"]), "wb" ) )  
#        pickle.dump(Ensamble_gene_prob, open( ''.join([self.name,"_T_1_test_Ensamble_gene_prob.p"]), "wb" ) )  
#        pickle.dump(gene_direct_prob, open( ''.join([self.name,"_gene_direct_prob.p"]), "wb" ) )  
#        pickle.dump(fin_gene_prob, open( ''.join([self.name,"_fin_gene_prob.p"]), "wb" ) )  
#        pickle.dump(Ensamble_gene_prob, open( ''.join([self.name,"_Ensamble_gene_prob.p"]), "wb" ) )  
        print('pikle of ', name, ' done :)')
