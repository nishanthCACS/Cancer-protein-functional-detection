# -*- coding: utf-8 -*-
"""
Created on %3-Mar-2019(10.57A.m)
...
f= open('Finalize_all_methods.csv',"w+")
...
for names in files:
    ...
    for ids in gene_direct_prob:
        ...
        if  ids[0] not in problemetric:
            f.write(name[-1] +',\t'+ uni_gene_to_id_gene_name(fin_selec,ids[0])+',\t' +  ids[0] +',\t'  +'Direct,\t'+ str(round(ids[1][0],2))  +',\t' + str(round(ids[1][1],2))+',\t' + str(round(ids[1][2],2))+',\t'+predicted+',\t' + name[0:-4]+'\n' )

    for ids in Ensamble_gene_prob:
        #insert the group details
        predicted = decider_prob(f, ids, minimum_prob_decide,1)
        if  ids[0] not in problemetric:
            f.write(name[-1] +',\t'+ uni_gene_to_id_gene_name(fin_selec,ids[0])+',\t' +  ids[0] +',\t'  +'Ensamble,\t'+ str(round(ids[1][0],2))  +',\t' + str(round(ids[1][1],2))+',\t' + str(round(ids[1][2],2))+',\t'+predicted+',\t' + name[0:-4]+'\n' )
f.close() 
#%%
def method(i):
    os.chdir('/')
...
f = open('uniprot-unigene_gene_edited.csv')
csv_f = csv.reader(f)
stack=[]
for row in csv_f:
    stack.append(row)
