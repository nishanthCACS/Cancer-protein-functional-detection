...
class retrieve_comp_results_not_valid_10_fold_NN:
    ...
    def __init__(self,model_name,nn_probability,saving_dir,ear_prob=True):
        self.saving_dir=saving_dir
        test_prob_dic={}
        test_prob_dic_checker={}
        for nn_probability_sel in [nn_probability]:
            os.chdir('/')
                ...
                for i in range(0,len(test_PDBs)):
                    for j in range(0,len(test_probabilities[i])):
                        if nth_fold==0:
                           ...
                           test_prob_dic_checker[test_PDBs[i][j]]=np.zeros(10)
                           test_prob_dic_checker[test_PDBs[i][j]][nth_fold]=1
                        else:
                            if test_prob_dic_checker[test_PDBs[i][j]][nth_fold]==0:
                                test_prob_dic_checker[test_PDBs[i][j]][nth_fold]=1
...
#        test_PDBs=list(set(test_PDBs+OG_test_pdbs+TSG_test_pdbs+Fusion_test_pdbs))
    ...
    def Helper_to_get_T_1_ROC_for_V88(self,satisfied_dir,PDB_list_icluded=False):
        '''
        This function helps to retrieve the results for Tier-1 ONGO, TSG, Fusion results
        '''
        os.chdir('/')    
        os.chdir(satisfied_dir)        
        OG_V2_pdbs = pickle.load(open("OG_10_fold_PDBs.p", "rb"))
        TSG_V2_pdbs = pickle.load(open("TSG_10_fold_PDBs.p", "rb"))  
        Fusion_V2_pdbs = pickle.load(open("Fusion_10_fold_PDBs.p", "rb")) 
                 
        ONGO_test_probs_numpy=self.given_PDBs_prob(OG_V2_pdbs,V_88=True)
        TSG_test_probs_numpy =self.given_PDBs_prob(TSG_V2_pdbs,V_88=True)
        Fusion_test_probs_numpy =self.given_PDBs_prob(Fusion_V2_pdbs,V_88=True)
        
        os.chdir('/')
        os.chdir(self.saving_dir)   
        np.save('OG_10_fold_probs.npy', ONGO_test_probs_numpy)
        np.save('TSG_10_fold_probs.npy', TSG_test_probs_numpy)
        np.save('Fusion_10_fold_probs.npy', Fusion_test_probs_numpy)
        pickle.dump(OG_V2_pdbs, open("OG_10_fold_PDBs.p", "wb"))  
        pickle.dump(TSG_V2_pdbs, open("TSG_10_fold_PDBs.p", "wb")) 
        pickle.dump(Fusion_V2_pdbs, open("Fusion_10_fold_PDBs.p", "wb"))  
        if PDB_list_icluded: 
            return OG_V2_pdbs,TSG_V2_pdbs,Fusion_V2_pdbs,ONGO_test_probs_numpy,TSG_test_probs_numpy,Fusion_test_probs_numpy
        else:
            return ONGO_test_probs_numpy,TSG_test_probs_numpy,Fusion_test_probs_numpy
...
class retrieve_comp_results_not_valid_10_fold_NN_ensemble(retrieve_comp_results_not_valid_10_fold_NN):
    """
    This class retrieve the 10-fold results of the ensamble metghod if the PDB not in validation set else return the validation probability
    """
    ...
    def __init__(self,model_name,nn_probability,saving_dir,ear_prob=True):
        ...
        for pdbs in test_PDBs:
            test_prob_dic[pdbs]=self.prob_fix_func(test_prob_dic[pdbs]/(10*len(nn_probability)))
