# -*- coding: utf-8 -*-
"""
Created on %29-Sep-2020 at 03.37p.m
...
class retrieve_comp_results_ens_10_fold_NN:
    """
    This class retrieve the 10-fold results and ensamble all
    """
    
    def __init__(self,model_name,nn_probability,nn_probability_sel_dirs,saving_dir):
        self.saving_dir=saving_dir
        test_prob_dic={}
        chk_ini=True
        for nn_probability_sel in nn_probability_sel_dirs:
            os.chdir('/')
            os.chdir(''.join([nn_probability,'/',nn_probability_sel]))
            for nth_fold in range(0,10):
            #nth_fold=1
                test_PDBs_given= pickle.load(open(''.join(["pdbs_test_",str(nth_fold),".p"]), "rb"))
                test_probabilities = pickle.load(open(''.join(["test_probabilities_",str(nth_fold),".p"]), "rb"))
                #go through the PDBs and retrie the results
                if chk_ini:
                    test_PDBs=test_PDBs_given
                    chk_ini=False
                else:
                    test_PDBs=[]
                    for key in test_PDBs_given:
                        if key not in list(test_prob_dic.keys()): 
