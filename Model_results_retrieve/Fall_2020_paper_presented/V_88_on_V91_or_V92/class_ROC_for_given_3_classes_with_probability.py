...
editted on %18-Nov-2020 at 11.12 p.m

mod_roc=True added on ROC_OG_Vs_TSG_Vs_Fusion function change on 
if you want to change the portion of FPR change the limit the plot limit by
           plt.xlim([0.0, 0.015])
                           ^
                           Here
"""
...
class ROC_for_given_3_classes_with_probability:
    ...
    def __init__(self,OG_test_probs_numpy,TSG_test_probs_numpy,Fusion_test_probs_numpy,minimum_prob_decide= 0.25,plot_on=True,bin_conf_mat=False):
        ...
        self.feedback=True#to give the feed back 
    ...
    def confusion_matrix_to_accuracy(self,confusion_matrix_test):
        '''
        This function create the confusion matrix's accuracy 
        if 
             the rows are predicted one
             coloumns are the actual one
        '''
        ONGO_acc=round(100*confusion_matrix_test[0,0]/sum(confusion_matrix_test[:,0]),2)
        TSG_acc=round(100*confusion_matrix_test[1,1]/sum(confusion_matrix_test[:,1]),2)
        Fusion_acc=round(100*confusion_matrix_test[2,2]/sum(confusion_matrix_test[:,2]),2)
        Overall_acc=round(100*(confusion_matrix_test[0,0]+confusion_matrix_test[1,1]+confusion_matrix_test[2,2])/sum(sum(confusion_matrix_test)),2)
        PDB_Accuracies=np.array([Overall_acc,ONGO_acc,TSG_acc,Fusion_acc])
    ...
    def ROC_OG_VS_Fusion(self,plot_on,conf_mat=False):
        ...
        if conf_mat:
            ...
            return self.ROC_plot(y_test,y_score,2,OG_Fusion_only=True,plot_on=plot_on),confusion_matrix_binary 
        else:
            return self.ROC_plot(y_test,y_score,2,OG_Fusion_only=True,plot_on=plot_on)      
    ...
    def ROC_TSG_VS_Fusion(self,plot_on,conf_mat=False):
        ...
        if conf_mat:
            ...
            confusion_matrix_binary[:,1]=[len(Fusion_y_test)-Fusion_correct,Fusion_correct]
            return self.ROC_plot(y_test,y_score,2,TSG_Fusion_only=True,plot_on=plot_on),confusion_matrix_binary     
        else:
            return self.ROC_plot(y_test,y_score,2,TSG_Fusion_only=True,plot_on=plot_on)      
        
    def ROC_OG_Vs_TSG_Vs_Fusion(self,plot_on,mod_roc=False):
        ...
        if not mod_roc:
            ...
            return self.mod_ROC_plot(y_test,y_score,3,plot_on=plot_on)
    ...
    def mod_ROC_plot(self,y_test,y_score,n_classes,OG_TSG_only=False,TSG_Fusion_only=False,OG_Fusion_only=False,plot_on=True):
        fpr = dict()
        tpr = dict()
        roc_auc = dict()
        for i in range(n_classes):
