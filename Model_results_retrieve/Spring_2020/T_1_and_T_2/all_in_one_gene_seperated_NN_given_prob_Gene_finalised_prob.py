...
def gene_seprated_results_finalise_fully_clean_21_SITE_MUST_old_threh(name,loading_NN_prob_dir,saving_dir_part,SITE_MUST,num_amino_acids,Tier=1):
    ...
    if SITE_MUST:
        ...
        train_new_gene_list = pickle.load(open(''.join([name,"_train_gene_id.p"]), "rb")) 
        test_new_gene_list = pickle.load(open(''.join([name,"_test_gene_id.p"]), "rb")) 
        51366
        working_dir_fraction = ''.join(['C:/Users/nishy/Documents/Projects_UL/Continues BIBM/Uniprot_Mapping/Spring_2020/weights_assign_methods/used_Fully_clean_models/T_',str(Tier),'/SITE/before_thresh/',str(num_amino_acids),'_aminoacid'])

    else:
        unigene_load_dir = 'C:/Users/nishy/Documents/Projects_UL/Continues BIBM/a_Results_for_model_train/Tier_1/clean/W_OUT_SITE'
#        os.chdir('/')
#        os.chdir(unigene_load_dir)
#        train_new_gene_list = pickle.load(open(''.join([name,"_train_unigene_details.p"]), "rb")) 
#        test_new_gene_list = pickle.load(open(''.join([name,"_test_unigene_details.p"]), "rb")) 
...
ONGO_train_gene_direct_prob, ONGO_train_fin_gene_prob, ONGO_train_Ensamble_gene_prob, ONGO_test_gene_direct_prob, ONGO_test_fin_gene_prob, ONGO_test_Ensamble_gene_prob = gene_seprated_results_finalise_fully_clean_21_SITE_MUST_old_threh("ONGO",saving_dir_NN_retrieved,saving_dir_primary_seq_part,SITE_MUST,num_amino_acids)
TSG_train_gene_direct_prob, TSG_train_fin_gene_prob, TSG_train_Ensamble_gene_prob, TSG_test_gene_direct_prob, TSG_test_fin_gene_prob, TSG_test_Ensamble_gene_prob = gene_seprated_results_finalise_fully_clean_21_SITE_MUST_old_threh("TSG",saving_dir_NN_retrieved,saving_dir_primary_seq_part,SITE_MUST,num_amino_acids)
Fusion_train_gene_direct_prob, Fusion_train_fin_gene_prob, Fusion_train_Ensamble_gene_prob, Fusion_test_gene_direct_prob, Fusion_test_fin_gene_prob, Fusion_test_Ensamble_gene_prob = gene_seprated_results_finalise_fully_clean_21_SITE_MUST_old_threh("Fusion",saving_dir_NN_retrieved,saving_dir_primary_seq_part,SITE_MUST,num_amino_acids)
#%%
