# -*- coding: utf-8 -*-
"""
Created on %(07-Nov-2018) 14.09Pm
...
class probability_weight_cal_results_from_NN_NOF:
    """
    ...
    def __init__(self,working_dir_fraction, working_dir_NN_prob, unigene_test_dir, saving_dir,name, minimum_prob_decide= 0.25):
        ...
        self.unigene_test_dir = unigene_test_dir
        self.name = name
        ...
        for row in csv_f:
            if cond:
                ...
                probabilities.append([float(row[1]),float(row[2])])
...
#                        p_Fusion = p_Fusion + pdb_temp[k]*probabilities[j][2]
        ...
        prob = [p_OG/(p_OG + p_TSG), p_TSG/(p_OG + p_TSG)]
        return  prob, whole[0]
    ...
    def calculate_prob(self):                
        ...
        for i in range(0,len(method_1_pikles)):
            print("Problemetric pikle: ",method_2_pikles[i] )
            ...
            prob_ens = [(prob_1[0] + prob_2[0] + prob_3[0])/3, (prob_1[1] + prob_2[1] + prob_3[1])/3]
    ...
    def decider_prob(self,f,ids, minimum_prob_decide,i):
        ...
        """ 
        if ids[i][0] > minimum_prob_decide and ids[i][1] > minimum_prob_decide:
           f.write( '\t' +', '+ "ONGO_TSG" + '\n') 
    ...
    def documentation_prob(self):
        ...
        f.write('Uni_gene_ID'+'\t'+', OG'+'\t'+ ', TSG'+ '\t' + 'Class' + '\n')
        
        for ids in gene_direct_prob:
            #insert the group details
            f.write(ids[0] + '\t' +',' + str(round(ids[1][0],2))  + '\t' +',' + str(round(ids[1][1],2)))
        ...
        for ids in fin_gene_prob:
            #insert the group details
            f.write(ids[0] + '\t' +',' + str(round(ids[2][0],2))  + '\t' +',' + str(round(ids[2][1],2)) + '\t')
        ...
        for ids in fin_gene_prob:
            #insert the group details
            f.write(ids[0] + '\t' +',' + str(round(ids[3][0],2))  + '\t' +',' + str(round(ids[3][1],2)))
