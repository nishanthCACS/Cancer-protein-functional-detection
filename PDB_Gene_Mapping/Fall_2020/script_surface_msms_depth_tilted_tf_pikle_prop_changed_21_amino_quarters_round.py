...
editted on 06-Nov-2020 at 10.09 p.m

"""
import pickle
import os
from  class_surface_msms_depth_tilted_tf_pikle_prop_changed_21_amino_quarters_round import surface_msms_depth_MOTIF_class_quat_round
...
#        del obj
    ...
    for pdb_name in train_pdb_ids_details_all:
        if pdb_name!='4MDQ':
#            print(pdb_name)
            obj = surface_msms_depth_MOTIF_class_quat_round(loading_pikle_dir, saving_dir_part_train, pdb_name,height=height)
            del obj
#    saving_dir_part_test=''.join([saving_dir_part,'/Test/',name])
    saving_dir_part_test=''.join([saving_dir_part,'/',name])
    ...
    for pdb_name in test_pdb_ids_details_all:
        if pdb_name!='4MDQ':
#            print(pdb_name)
            obj = surface_msms_depth_MOTIF_class_quat_round(loading_pikle_dir, saving_dir_part_test, pdb_name,height=height)
            del obj    
#%%
height=200
name = "ONGO"
