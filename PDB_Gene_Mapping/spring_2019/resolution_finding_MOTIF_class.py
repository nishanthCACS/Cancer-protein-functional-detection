# -*- coding: utf-8 -*-
"""
Created on %25-January-2018(12.53pm)
...
class resolution_finding_MOTIF:
    """
    ...
    def surface_select_atom(self, coordinates, resolution_xy, resolution_xy_z):
        """
...
#        range_of_xy_z= atom_angle_range(angle_xy_z,2*resolution_xy_z)
    ...
    def resolution_mumpy_pikle(self):
        """
        ...
        coordinates,amino_acid, MOTIF_indexs_all = self.retrieve_c_alpha()
#        not_sat_motifs = np.zeros((4,4))
#        resolution_xy_list = [3,6,15,30]
        not_sat_motifs = np.zeros((6,5))
        resolution_xy_list = [3,4,5,6,15,30]
        resolution_xy_z_list = [1,2,3,4,5]
#        total_motif_atoms = 0
        for i in range(0,6):
            for j in range(0,5):
        #        total_motif_atoms = 0
                sel_atoms= self.surface_select_atom(coordinates, resolution_xy_list[i], resolution_xy_z_list[j])
        ...
        np.save("".join([self.pdb_name,"_resolution.npy"]), not_sat_motifs)
        
    def resolution_per_mumpy_pikle(self):
        """
        Choose the angle range as 
        theta   : 9, 18, 36, 60
        Phi     : 4, 9, 18, 30 and check howmany MOTIF's atoms placed into Surface atoms
                (by considering 
                         minimum angle difference in the same group of MOTIF > Phi considered)
        then create howmany MOTIF group's atoms persentage among the group not satisfied by these catergories
        #this canbe represented as np-array
        ...
        The difference from the function "resolution_mumpy_pikle"
        this 
        calculate the percentage of MOTIF atoms missed among the group
            this make if the group contain 4 atoms loosing 1 atom make 25% loss
            if the group contain 20 atoms and loosing 1 atom makes 5% loss
        Then finally averaged among all MOTIF's group
        This averaging consider all MOTIF groups even if the group doesn't loose
        any of the atoms
...
#        total_motif_atoms = 0
        ...
        #        total_motif_atoms = 0
                ...
                for m_g in MOTIF_indexs_all:
                    c_mg = 0
                    if len(m_g)>0:#since there is devision t
                        for m_i in m_g:
                            if m_i not in sel_atoms:
                                c_mg = c_mg + 1 
        ...
        os.chdir(self.saving_dir)
        np.save("".join([self.pdb_name,"_per_resolution.npy"]), not_sat_motifs)