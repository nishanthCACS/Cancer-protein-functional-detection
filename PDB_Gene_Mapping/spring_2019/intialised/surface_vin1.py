...
def retrieve_c_alpha(stack):
    ...
    #            print("perfect :)")
        ...
        return coordinates,amino_acid,chain_id_info,resSeq, MOTIF_indexs_all
    else:
...
def retrieve_info(pdb_file):
    ...
    """
    ca_depth_all, res_depth_all,keys,surface = depth_calculator(pdb_file)
    stack= stack_retrieve(pdb_file)
    coordinates,amino_acid,chain_id_info,resSeq, MOTIF_indexs_all = retrieve_c_alpha(stack)
    
    fin_res_depth_all=[]
    fin_ca_depth_all=[]
    for i in range(0,len(coordinates)):
        for j in range(0,len(keys)):
            if keys[j][0][0]==chain_id_info[i] and keys[j][1][1]==int(resSeq[i]):
                fin_res_depth_all.append(res_depth_all[j])
                fin_ca_depth_all.append(ca_depth_all[j])
   
    return MOTIF_indexs_all,coordinates,amino_acid,fin_res_depth_all,fin_ca_depth_all,surface
#%%
name = "ONGO"
...
pdb_name = "3QS7"
pdb_source_dir_part = "C:/Users/nishy/Documents/Projects_UL/Continues BIBM/Results_for_R/ONGO_R"
os.chdir('/')
os.chdir(pdb_source_dir_part)
pdb_file = ''.join([pdb_name,".pdb"])
               
#MOTIF_indexs_all,coordinates,amino_acid,fin_res_depth_all,fin_ca_depth_all,surface=retrieve_info(pdb_file)      
ca_depth_all, res_depth_all,keys,surface = depth_calculator(pdb_file)
stack= stack_retrieve(pdb_file)
coordinates,amino_acid,chain_id_info,resSeq, MOTIF_indexs_all = retrieve_c_alpha(stack)
#%%
