...
#%% useful functions
def select_pdb_with_high_length(un_overlap):
    """
...
def method_1_find_over_lab_with_sel(sel_PDB_id, un_overlap, high_length): 
    """
...
#%%%
def method_1_rest(start_end_position_t, pdb_ids_for_gene_t): 
    """
    method_1 discription
    
    It chooses the highest length PDB_id for fraction calculation
    
    1> choose highest length PDB_id is selected
...
#        c=0
    ...
    while high_length > 0:
#            print("count: ",c)
        high_length, sel_PDB_id = select_pdb_with_high_length(un_overlap)
#            print("sel_PDB_id: ",sel_PDB_id)
#            print("high_length: ",high_length)
        if  high_length == 0:
            break
        un_overlap, over_lapped_sat = method_1_find_over_lab_with_sel(sel_PDB_id, un_overlap, high_length)
...
#%%
def method_2_rest(pdb_ids_for_gene_t, pdb_ids_matched_with_unique, avail_start, sort_start_positions_index, avail_end, start_end_position_t,i):
    """
    use dynamic programming to cover maximum length of the gene sequence with non_over lapping PDBs
...
#% finaly choose the last element from the indexes choosen that is the best
##then formalize the results same as Method_1
    """This part is commented just before"""
#lengths_considered = []
#fin_fractions = []
#for index_groups in indexes_choosen_cov[-1]:
#    fin_fractions_temp = []
...
for i in range(0,len(fin_PDB_ids_start_end_position)):
    ...
    if len(start_end_position_t)>1:     
        ...
        Method_1
        """
        whole = method_1_rest(start_end_position_t, pdb_ids_for_gene_t)
        ...
        Method_2
        """
        whole = method_2_rest(pdb_ids_for_gene_t,pdb_ids_matched_with_unique, avail_start, sort_start_positions_index, avail_end, start_end_position_t,i)
        pickle.dump(whole, open( ''.join([name,"_method_2_", fin_uni_gene[i],".p"]), "wb" ) )  
    
        """
        Method_3
        """
        method_3_rest(pdb_ids_matched_with_unique, avail_start, sort_start_positions_index, avail_end, pdb_ids_for_gene_t, fin_uni_gene, i)
