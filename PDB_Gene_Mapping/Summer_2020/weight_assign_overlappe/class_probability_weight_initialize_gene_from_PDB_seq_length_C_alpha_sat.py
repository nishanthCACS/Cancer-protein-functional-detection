...
class probability_weight_initialize_gene_from_PDB_seq_length_C_alpha_sat:
    """
    ...
    """
    def __init__(self,working_dir,saving_dir,name,Tier,amino_acids,threshold_length = 81,Not_Fully_clean=False):
...
#        SITE_satisfied_all=list(SITE_satisfied_all+overlap)
...
#                                   print("Not_satisfied: ",id_t[0])
        ...
        pickle.dump(fin_uni_gene, open( ''.join([name,"_satisfied_Gene_ID.p"]), "wb" ) ) 
        pickle.dump(fin_PDB_ids, open( ''.join([name,"_satisfied_PDBs.p"]), "wb" ) ) 
                ...
                if maximum_cov < cov[-1] - avail_end[indexes_choosen_cov[-1][-1]]  + avail_end[sort_start_positions_index[j]]:
                    ...
                    if len(indexes_choosen_cov[-1])==1 and avail_start[sort_start_positions_index[j]]==avail_start[indexes_choosen_cov[-1][-1]]:#to fix the same satrting postion but different ending position
                        ...
                        if avail_start[sort_start_positions_index[j]]==avail_start[indexes_choosen_cov[-1][-1]]:#to fix the same satrting postion but different ending position
                            ...
                            indexes_choosen_cov_change = self.size_index_chhosen_checker(indexes_choosen_cov)
                            if avail_start[sort_start_positions_index[j]]<= avail_end[indexes_choosen_cov[indexes_choosen_cov_change[-2]][-1]]:
                                ...
                                while (p>-1 and avail_start[indexes_choosen_cov[p][-1]]<= avail_start[sort_start_positions_index[j]]<= avail_end[indexes_choosen_cov[p][-1]]):
                                   p_need=p
                                   p=p-1
    ...
    def size_index_chhosen_checker(self,list_1):
        size_change_places=[]
    ...
    def method_whole_finalize(self):
        ...
        for i in range(0,len(fin_PDB_ids_start_end_position)):
            ...
            if len(start_end_position_t)>1:     
                ...
                Method_3
                """
                print("fin_uni_gene: i ,",i,' ',fin_uni_gene[i]," name: ",name )
