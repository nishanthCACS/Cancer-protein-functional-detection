...
class probability_weight_initialize_gene_from_PDB_seq_length_SITE_MUST:
    """
    ...
    """
    def __init__(self,working_dir,saving_dir,name,threshold_length = 81,SITE_MUST = False,site_dir = 'bb'):
                    ...
                    for j in range(0,len(id_t[2])):
                            if id_t[2][j]=="-":
                               if (int(id_t[2][j+1:len(id_t[2])])-int(id_t[2][0:j]))+1 > threshold_length:
                                   if SITE_MUST:# check the SITE_SAT condiotion given otherwise just add all PDBs as it is
